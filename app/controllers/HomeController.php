...
class HomeController extends BaseController {
    ...
    private $wxBaseUrl = 'https://open.weixin.qq.com/connect/oauth2/authorize?';
        //获取游戏页面
...
//            }
    ...
    //                      $code = Input::get('code');
    //                      if(!$code) {
    //                          return Redirect::to("https://open.weixin.qq.com/connect/oauth2/authorize?appid=$this->appid&redirect_uri=https%3a%2f%2fwx.idsbllp.cn%2fgame%2fgame%2findex.php%2fcqupt-group-photo&response_type=code&scope=snsapi_userinfo&state=sfasdfasdfefvee#wechat_redirect");
    //                      }
                $openId = Input::get('openid');
                $img = Input::get('headimgurl');
                if (!$openId)
                    return Redirect::to('//wx.idsbllp.cn/MagicLoop/index.php?s=/addon/Api/Api/oauth&redirect='.urlencode('https://'.$_SERVER['HTTP_HOST'].'/game'.$_SERVER['REQUEST_URI']));
                      Session::put('openid', $openId);
                      Session::put('img', $img);
    ...
    //                  }
                  if (Session::get('openid') || Input::get('openid')) {
                      //Session::put('openid', Input::get('openid'));
                      $ticket = $this->JSSDKSignature();
                      return View::make('twolearnonedo.index')->with('openid', Session::get('openid'))->with('ticket', $ticket)->with('appid', $this->appid);
                  }
    ...
    //                  $uri = 'https://wx.idsbllp.cn/MagicLoop/index.php?s=/addon/Api/Api/oauth&redirect='.urlencode('https://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
            ...
            case 'learnpartyconstitution':
                  if(Session::get('openid') || Input::get('openid')) {
                      //Session::put('openid', Input::get('openid'));
                      $ticket = $this->JSSDKSignature();
                      return View::make('party.index')->with('openid', Session::get('openid'))->with('ticket', $ticket)->with('appid', $this->appid);
                  }
    //                  $uri = 'https://wx.idsbllp.cn/MagicLoop/index.php?s=/addon/Api/Api/oauth&redirect='.urlencode('https://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
                  $uri = '//wx.idsbllp.cn/MagicLoop/index.php?s=/addon/Api/Api/oauth&redirect='.urlencode('https://'.$_SERVER['HTTP_HOST'].'/game'.$_SERVER['REQUEST_URI']);
		          return Redirect::to($uri);
    	    case 'heroesoftimes':
    		    ...
    		    $ticket=$this->JSSDKSignature();
    		    return View::make('heroesoftimes.index')->with('avatar',Session::get('img'))->with('ticket',$ticket)->with('appid',$this->appid);
            default:
    ...
    private function authorize($game) {
        $now = 'https://wx.idsbllp.cn/game/game/index.php/' . $game;
        $code = Input::get('code');
        $openid = Session::get('openid');
        if (!$openid && !$code) {
            header('Location: https://open.weixin.qq.com/connect/oauth2/authorize?' . $this->getUrlParam($now));
            exit();
        }

        if (!!$code && !$openid) {
            $re = $this->myCurl('https://api.weixin.qq.com/sns/oauth2/access_token?appid=wx81a4a4b77ec98ff4&secret=872a908ec98bd92f8db811eba2a83236&code='  . $code  . '&grant_type=authorization_code');
            if (empty($re['errcode'])) {
                Session::set('openid', $re['openid']);
            } else {
                header('Location: https://open.weixin.qq.com/connect/oauth2/authorize?' . $this->getUrlParam($now));
                exit();
            }
        ...
        if (!$openid) {
            header('https://open.weixin.qq.com/connect/oauth2/authorize?' . $this->getUrlParam($now));
        }
...
//        curl_setopt ( $ch, CURLOPT_POSTFIELDS, $params );
    ...
    private function getUrlParam($url) {
        $end = '#wechat_redirect';
        $arr = array(
    ...
    public function verify() {
        $openid = Session::get('openid');
	if (!$openid) {
            return Response::make('no auth', 401);
        }
...
//                if($save['openid'] != '')
        ...
        public function tlodquestion(){
            ...
            foreach ($question as &$value){
                $value->pic = '//wx.idsbllp.cn/game/game/images/twolearnonedo/'.$value->pic;
        ...
        public function hotquestion(){
            $question = DB::table('heroesoftimes_question')->orderBy(DB::raw('RAND()'))->take(9)->get();
            foreach ($question as &$value){
                $value->pic = '//wx.idsbllp.cn/game/game/images/heroesoftimes/'.$value->pic;
                $value->nameLength = mb_strlen($value->answer, 'utf-8');
            }
            return [
                'status' => 200,
        ...
        public function hotRecord(){
            ...
            $table = DB::table('heroesoftimes_score');
            $row = $table->where('openid', '=', $openid)->first();
            if (!$row) {
                $table->insert(['openid' => $openid]);
                $row = $table->where('openid', '=', $openid)->first();
            ...
            $result = DB::select('SELECT * FROM (SELECT obj.openid,@grank:=@grank+1 as rank FROM (SELECT openid,`right`,time FROM heroesoftimes_score ORDER BY `right` DESC,time)obj,(SELECT @grank:=0)r)a WHERE openid ="'.$openid.'"');
            $rank = $result[0]->rank;
            //$rank = $rank * 15;
            // if ($rank < 60) {
            //     $rank = $rank * 9;
    ...
    //
        ...
        public function JSSDKSignature(){
            ...
            $data['timestamp'] = time();
            $data['url'] = 'https://'.$_SERVER['HTTP_HOST']. '/game' .$_SERVER['REQUEST_URI'];//生成当前页面url
